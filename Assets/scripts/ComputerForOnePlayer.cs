...
public class ComputerForOnePlayer : MonoBehaviour
{
    ...
    IEnumerator move(int drawnPlayer)
    {
        int index = Random.Range(0, hands.hands[dP].Count);
        cardIndex = hands.hands[dP][index];
    ...
    void OnGUI()
    {
        if (flag)
        {
            if (GUI.Button(new Rect(300, 10, 100, 20), "CPU turn draw"))
            {
                ...
                {
                    if (moveFlag) return;
                    StartCoroutine(move(dP));
                    StopCoroutine(move(dP));
                }
            }
        }
    }
    ...
    void Update()
    {
        if (moveFlag)
        {
            card.transform.Translate(0, Time.deltaTime * 0.8f, 0);
        }
